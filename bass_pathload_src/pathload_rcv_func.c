...
//tfang
void AverageProbeStream(int spike_begin, int spike_end, double* sendgap, double* recvgap){
  double sum_sendgap = 0;
  double sum_recvgap = 0;

  if(spike_begin < spike_end){   //spike_begin == start index,  spike_end == end index.
    for(int i = spike_begin; i <= spike_end; i++){
      //is this true
      sum_sendgap += sendgap[i];
      sum_recvgap += recvgap[i];
    }
    for(int i = spike_begin; i <= spike_end; i++){
      sendgap[i] = sum_sendgap/(spike_end - spike_begin + 1);
      recvgap[i] = sum_recvgap/(spike_end - spike_begin + 1);
    }
  }

}

void SpikeRemoval(double* sendgap, double* recvgap, int N){
  int i;
  int spike_state;
  int spike_begin;
  int spike_end;
  ...
  if(recvgap[0] > recvgap[1] + SPIKE_DOWN){
    spike_begin = 0;
    spike_max = recvgap[0];
    spike_state = SPIKE_VALID;
    i = 1;
  }else{  // Lisong
    spike_begin = 0;
    spike_max = recvgap[0];
  ...
  for(;i <= N-1; i++){
    switch(spike_state){
      ...
      case NONE:
      //line 19
      if((recvgap[i] + SPIKE_UP) < recvgap[i+1]){
        spike_end = i;
        AverageProbeStream(spike_begin, spike_end, sendgap, recvgap);
        spike_state = SPIKE_PENDING;
        spike_begin = i+1;
        spike_max = recvgap[spike_begin];
      }
      break;
      //spike check for pending
      case SPIKE_PENDING:
      if(spike_max < recvgap[i]){
        spike_max = recvgap[i];
      }
      if((recvgap[i] + SPIKE_DOWN) < spike_max){
        spike_state = SPIKE_VALID;
      } else {
        break;
      }
      //spike check for valid
      case SPIKE_VALID:
      if((recvgap[i] + SPIKE_UP) < recvgap[i+1]){
        spike_end = i;
        spike_state = SPIKE_PENDING;
        spike_max = recvgap[i+1];
      } else {
        //if(recvgap[i] == recvgap[N-2]){   // Lisong
        //  spike_end = i;
        //}
        break;
      }
  ...
  //please check where to call the method
  spike_end = N-1;  // Lisong
  AverageProbeStream(spike_begin, spike_end, sendgap, recvgap);
}
...
K=%ldpackets, T=%ldusec\n",tr, cur_pkt_sz , stream_len,time_interval);
  ...
  while ( stream_cnt < num_stream  )
  {
    ...
    //start BASS algorithm  (comment the area between start BASS and end BASS to remove the BASS effect)
    double sendgap[stream_len-1];
    double recvgap[stream_len-1];
    for(int bass = 0; bass<stream_len-1; bass++){
    	sendgap[bass] = snd_tm[bass+1] - snd_tm[bass];
    	recvgap[bass] = arrv_tm[bass+1] - arrv_tm[bass];
    }  
    SpikeRemoval(sendgap, recvgap, stream_len-1);
    
    for(int bass = 0; bass<stream_len-1; bass++){
    	snd_tm[bass+1] = snd_tm[bass] + sendgap[bass];
    	arrv_tm[bass+1] = arrv_tm[bass] + recvgap[bass];
    }   
